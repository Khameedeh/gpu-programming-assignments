...
// BUG FIX: Changed signal to broadcast to wake all waiting consumers
void* producer(void* arg) {
    ...
    for (int i = 0; i < ITEMS_TO_PRODUCE; i++) {
        ...
        // FIX: Changed from signal to broadcast
        // This ensures all waiting consumers are woken, not just one
        // Multiple consumers might be waiting, and we want all of them to check the condition
...
// BUG FIX: Changed signal to broadcast
void* consumer(void* arg) {
    ...
    while (1) {
        ...
        // FIX: Changed from signal to broadcast
        // Wake all waiting producers to check if buffer has space
